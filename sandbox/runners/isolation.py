...
async def tmp_overlayfs():
    ...
    # The overlay was mounted via sudo so the upper layer is owned by root —
    # writing /etc/hosts into the merged tree triggers a copy-up the unprivileged
    # process can't perform. Use sudo to match the mount privilege level.
    # `cp -L --remove-destination` resolves the symlinks (e.g. resolv.conf →
    # /run/systemd/resolve/...) which don't exist inside the chrooted tree,
    # and overwrites the symlink target in the overlay.
    await execute_command(['sudo', 'cp', '-L', '--remove-destination', '/etc/hosts', f'{merged_dir}/etc/hosts'])
    await execute_command(['sudo', 'cp', '-L', '--remove-destination', '/etc/resolv.conf', f'{merged_dir}/etc/resolv.conf'])
...
def _is_cgroup_v2() -> bool:
    """Cgroup v2 has a unified hierarchy with cgroup.controllers file at root,
    and lacks the per-controller subdirectories (/sys/fs/cgroup/memory etc.)
    that the v1-style cgcreate/cgset commands here rely on."""
    return os.path.exists('/sys/fs/cgroup/cgroup.controllers') and \
        not os.path.exists('/sys/fs/cgroup/memory')
...
async def tmp_cgroup(mem_limit: Optional[str] = None, cpu_limit: Optional[float] = None):
    ...
    if _is_cgroup_v2():
        # The v1-style `cgcreate -g memory:...` API isn't applicable on cgroup
        # v2 (Ubuntu 22+, RHEL 9+). Yield empty groups so the caller still
        # benefits from chroot + netns + pid-ns security isolation; we just
        # forgo resource limits. The original code's hard requirement that at
        # least one limit be set is also dropped here.
        logger.warning(
            'cgroup v2 detected — sandbox lite mode running without resource limits '
            '(chroot + netns + pidns isolation still active)'
        )
        yield groups
        return
