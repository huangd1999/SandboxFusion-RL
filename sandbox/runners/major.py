...
# Compile cache.
#
# Validator workflows fire the same source against many test cases serially
# (one /run_code request per test). Without a cache we re-run g++/javac for
# every test, which dominates wall time for cpp/java. We hash (lang, code)
# and persist the compiled artifacts under a stable directory so subsequent
# requests skip compile entirely. Each run still uses a fresh tmpdir for
# stdin/stdout isolation; we just copy the cached binary in.
#
# Failed compiles are NOT cached — we delete the cache_dir on failure so a
# fixed source compiles next time. Cross-worker races are benign: at worst
# two workers compile the same source once each.
# ---------------------------------------------------------------------------
_COMPILE_CACHE_ROOT = '/tmp/sandbox_compile_cache'
os.makedirs(_COMPILE_CACHE_ROOT, exist_ok=True)
_compile_locks: Dict[str, asyncio.Lock] = {}


def _get_compile_lock(key: str) -> asyncio.Lock:
...
def _cache_key(*parts) -> str:
    ...
    for p in parts:
        if p is None:
            p = ''
        h.update((p if isinstance(p, str) else str(p)).encode('utf-8'))
        h.update(b'\x00')
    return h.hexdigest()[:24]


def _is_compile_ok(result: CodeRunResult) -> bool:
    cr = result.compile_result
    if cr is None:
        # No compile step ran (e.g. cache hit path) — treat as OK.
        return True
    return cr.status == CommandRunStatus.Finished and cr.return_code == 0
...
async def run_cpp(args: CodeRunArgs) -> CodeRunResult:
    ...
    key = _cache_key('cpp17', flags_str, args.code)
    cache_dir = os.path.join(_COMPILE_CACHE_ROOT, f'cpp_{key}')
    cached_binary = os.path.join(cache_dir, 'test')

    # Compile once per (code, flags) combo; subsequent calls skip compile.
    lock = _get_compile_lock(key)
    async with lock:
        if not os.path.exists(cached_binary):
            os.makedirs(cache_dir, exist_ok=True)
            src_path = os.path.join(cache_dir, 'main.cpp')
            with open(src_path, 'w') as f:
                f.write(args.code)
            compile_cmd = f'g++ -std=c++17 main.cpp -o test {flags_str}'
            compile_res = await run_command_bare(compile_cmd,
                                                 args.compile_timeout,
                                                 None,
                                                 cache_dir,
                                                 {})
            if not (compile_res.status == CommandRunStatus.Finished and compile_res.return_code == 0):
                # Don't cache failed compile.
                shutil.rmtree(cache_dir, ignore_errors=True)
                return CodeRunResult(compile_result=compile_res, run_result=None, files={})

    # Cached binary exists — copy it into a fresh tmpdir for the actual run
    # so stdin / restored files / fetch_files are isolated per request.
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        run_binary = os.path.join(tmp_dir, 'test')
        shutil.copy2(cached_binary, run_binary)
        os.chmod(run_binary, 0o755)
        return await run_commands(None, './test', tmp_dir, {}, args)
...
async def run_java(args: CodeRunArgs) -> CodeRunResult:
    ...
    # Codeforces submissions often declare a public class with a name other
    # than Main (e.g. `public class A2 {}`). javac requires the source file
    # basename to match the public class name.
    class_name = find_java_public_class_name(args.code) or 'Main'
    key = _cache_key('java', class_name, args.code)
    cache_dir = os.path.join(_COMPILE_CACHE_ROOT, f'java_{key}')
    cached_class = os.path.join(cache_dir, f'{class_name}.class')

    base_jars = ['.', 'javatuples-1.2.jar']
    base_cp = ':'.join(base_jars)

    lock = _get_compile_lock(key)
    async with lock:
        if not os.path.exists(cached_class):
            os.makedirs(cache_dir, exist_ok=True)
            shutil.copy2(os.path.join(deps_dir, 'javatuples-1.2.jar'), cache_dir)
            src_path = os.path.join(cache_dir, f'{class_name}.java')
            with open(src_path, 'w') as f:
                f.write(args.code)
            compile_cmd = f'javac -cp {base_cp} {class_name}.java'
            compile_res = await run_command_bare(compile_cmd,
                                                 args.compile_timeout,
                                                 None,
                                                 cache_dir,
    ...
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        # Copy compiled artifacts (.class files + bundled jar) to fresh run dir.
        for name in os.listdir(cache_dir):
            if name.endswith('.java'):
                continue  # don't ship source, not needed at runtime
            shutil.copy2(os.path.join(cache_dir, name), os.path.join(tmp_dir, name))
        restore_files(tmp_dir, args.files)
        jars = list(base_jars) + [fn for fn in args.files.keys() if fn.endswith('.jar')]
        cpargs = f'-cp {":".join(jars)}'
        return await run_commands(None, f'java {cpargs} -ea {class_name}', tmp_dir, {}, args)
